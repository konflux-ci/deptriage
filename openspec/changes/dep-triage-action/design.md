...
**[Auto-approve via labels]** → The action applies `approved`/`lgtm` labels for eligible patches, minors, and non-gomod digests. For repos with external merge-bot automation (e.g., Prow/Tide), these labels are sufficient — the bot merges after CI passes. For repos without external merge bots, the `auto-merge` flag enables direct merge via the GitHub API (see Decision #16). Auto-approve applies to patches, minors, and non-gomod digests (not major), only when no risk hints are detected, and is configurable via `--auto-approve` flag (default: off). Gomod digest bumps are excluded because pseudo-versions have no semver guarantees. Renovate `pinDigest` updates (first-time SHA pinning) are classified as patches.

**[Formal review auto-approve risk]** → Auto-approving low-risk PRs could let a subtle breaking change through. Mitigation: auto-approve for patches and minors (not major or gomod digests), and only when no risk hints are detected. Configurable via `--auto-approve` flag (default: off).
